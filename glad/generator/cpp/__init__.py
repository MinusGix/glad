...
def get_debug_impl(command, command_code_name=None):
    ...
    if not is_void_ret:
        ret = DebugReturn(
            '{} ret;\n    '.format(type_to_cpp(command.proto.ret)),
            'ret = ',
            'return ret;'
        )
...
class CPPGenerator(JinjaGenerator):
    DISPLAY_NAME = 'C++17'

    TEMPLATES = ['glad.generator.cpp']
    Config = CPPConfig
    ...
    def get_templates(self, spec, feature_set, config):
        header = 'include/glad/{}.hpp'.format(feature_set.name)
        source = 'src/{}.cpp'.format(feature_set.name)

        templates = list()

        if config['HEADER_ONLY']:
            templates.extend([
                ('header_only.hpp', header)
            ])
        else:
            templates.extend([
                ('{}.hpp'.format(spec.name), header),
                ('{}.cpp'.format(spec.name), source)
            ])
