...
{{ command.proto.ret|type_to_cpp }} {{ command.name }}({{ command.params|params_to_cpp }});
{% endcall %}
{% endfor %}
{% endmacro %}
...
{% for command in commands %}
{% call protect(command) %}
typedef {{ command.proto.ret|type_to_cpp }} (GLAD_API_PTR *{{ command.name|pfn }})({{ command.params|params_to_cpp }});
{% endcall %}
{% endfor %}
{% endmacro %}
