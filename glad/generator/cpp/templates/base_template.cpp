...
{% block glad_include %}
#include <glad/{{ feature_set.name }}.hpp>
{% endblock %}
{% endif %}

{% include 'impl_util.cpp' %}
{% endblock %}
...
{% if options.on_demand %}
static {{ command.proto.ret|type_to_cpp }} GLAD_API_PTR glad_on_demand_impl_{{ command.name }}({{ command.params|params_to_cpp }}) {
...
static {{ command.proto.ret|type_to_cpp }} GLAD_API_PTR glad_debug_impl_{{ command.name }}({{ impl.impl }}) {
